...
test.describe('Path /search/granules/granule-details', () => {
  test('granule loads correctly', async ({ page }) => {
    ...
    await page.route(/collections.json/, (route) => {
      ...
      expect(route.request().postData()).toEqual('has_granules_or_cwic=true&include_facets=v2&include_granule_counts=true&include_has_granules=true&include_tags=edsc.*,opensearch.granule.osdd&page_num=1&page_size=20&sort_key[]=has_granules_or_cwic&sort_key[]=-usage_score')
