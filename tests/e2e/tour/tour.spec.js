...
test.describe('Joyride Tour Navigation', () => {
  ...
  test('should navigate through the Joyride tour', async ({ page }) => {
    ...
    let spotlight = page.locator('.react-joyride__spotlight[data-test-id="spotlight"]')
    await expect(spotlight).toBeVisible()

    // Get and verify the position and size of the highlighted section
    let spotlightRect = await getSpotlightPositionAndSize(spotlight)
    ...
    spotlight = page.locator('.react-joyride__spotlight[data-test-id="spotlight"]')
    await expect(spotlight).toBeVisible()

    spotlightRect = await getSpotlightPositionAndSize(spotlight)
    expect(spotlightRect).toEqual({
      left: -10,
      top: 108,
      width: 60,
