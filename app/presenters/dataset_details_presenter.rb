class DatasetDetailsPresenter < DetailsPresenter
  def initialize(dataset, collection_id=nil, token=nil, env='ops')
    ...
    @dataset.osdd_url = "#{opensearch_url}?utf8=%E2%9C%93&clientId=#{Rails.configuration.opensearch_client_id}&shortName=#{URI.escape(@dataset.short_name)}&versionId=#{URI.escape(@dataset.version_id)}&dataCenter=#{URI.escape(@dataset.archive_center)}&commit=Generate#{url_token}"
