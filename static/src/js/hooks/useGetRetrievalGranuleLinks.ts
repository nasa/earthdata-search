...
export const useGetRetrievalGranuleLinks = ({
  ...
  const [fetchGranuleLinks, { data = {}, loading }] = useLazyQuery(GET_RETRIEVAL_GRANULE_LINKS)
  ...
  useEffect(() => {
    ...
    const noLinksReturned = isEmpty(browse) && isEmpty(download) && isEmpty(s3)

    // If the retrieval is not done and links were returned, save the links and fetch the next page
    if (!done && !noLinksReturned) {
      // Update the percent done and current page
      setPercentDone((((currentPageRef.current) / totalPages) * 100).toFixed())
