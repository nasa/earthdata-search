...
describe('granule map events', () => {
  ...
  test('clicking on a granule sets that granule as sticky on the map', () => {
    ...
    expect(eventEmitterEmitMock).toHaveBeenCalledTimes(1)
    expect(eventEmitterEmitMock).toHaveBeenCalledWith('map.layer.C1219248410-LANCEMODIS.focusGranule', { granule: granules.find((g) => g.id === 'G1924512983-LANCEMODIS') })

    expect(setGranuleId).toHaveBeenCalledTimes(1)
    expect(setGranuleId).toHaveBeenCalledWith('G1924512983-LANCEMODIS')
  })
  ...
  test('clicking on a focused granule removes that granule as sticky on the map', () => {
    ...
    expect(setGranuleId).toHaveBeenCalledTimes(1)
    expect(setGranuleId).toHaveBeenCalledWith(null)
  })
})
...
describe('granule metadata', () => {
  test('returns the producerGranuleId as title', () => {
    ...
    const granule = granules.find((g) => g.id === 'G1924512983-LANCEMODIS')

    expect(granulesList[0].title).toEqual(granule.producerGranuleId)
    expect(granulesList[0].thumbnail).toEqual(granule.thumbnail)
  })
  ...
  test('returns the title as title if producerGranuleId does not exist', () => {
    ...
    expect(granulesList[0].title).toEqual('C1597928934-NOAA_NCEI:GHRSST-VIIRS_N20-OSPO-L2P.20181201112000-OSPO-L2P_GHRSST-SSTsubskin-VIIRS_N20-ACSPO_V2.60-v02.0-fv01.0.nc')
  })
  ...
  test('returns the s3 links if they exist', () => {
    ...
    const { granulesList } = setup({ granules })
