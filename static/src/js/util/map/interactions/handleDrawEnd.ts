...
const handleDrawEnd = (
  params: {
    /** The OpenLayers drawing interaction */
    ...
    /** Callback to handle the end of the drawing */
    onDrawEnd?: (geometry: Geometry | undefined) => void
    /** Callback to toggle the drawing new layer */
    onToggleDrawingNewLayer: (isDrawingNewLayer: boolean) => void
    /** The current projection code */
    projectionCode: keyof typeof crsProjections
    /** The current spatial type */
    spatialType: string
  },
  event: DrawEvent
) => {
  ...
  if (spatialType === spatialTypes.BOUNDING_BOX) {
    ...
    const start = (geometries[1] as Point).transform(
      crsProjections[projectionCode],
      crsProjections[projectionCodes.geographic as keyof typeof crsProjections]
    ).getCoordinates()
    const end = (geometries[2] as Point).transform(
      crsProjections[projectionCode],
  ...
  if (spatialType === spatialTypes.CIRCLE) {
    ...
    if (projectionCode === projectionCodes.geographic) {
      ...
      const [polygon, point] = (geographicCoordinates as GeometryCollection).getGeometries()

      const circleCenter = (point as Point).getCoordinates()
      const lastCoord = (polygon as Polygon).getLastCoordinate()
      ...
      const circleCenter = (geographicCoordinates as Circle).getCenter()
      const lastCoord = (geographicCoordinates as Polygon).getLastCoordinate()
  ...
  if (spatialType === spatialTypes.POINT) {
    ...
    const geographicCoordinates = (geometry as Point).transform(
      crsProjections[projectionCode],
      crsProjections[projectionCodes.geographic]
    )
  ...
  if (spatialType === spatialTypes.POLYGON) {
    ...
    const geographicCoordinates = (geometry as Polygon).transform(
      crsProjections[projectionCode],
      crsProjections[projectionCodes.geographic]
    )
