...
const correctedReflectance = ({
  ...
  const layer = new TileLayer({
    className: 'edsc-map-corrected-reflectance-layer',
    source: new XYZ({
      attributions,
      ...
      tileSize: 512,
      // Correct URL format with proper string template
      url: `https://gibs.earthdata.nasa.gov/wmts/${gibsProjection}/best/MODIS_Terra_CorrectedReflectance_TrueColor/default/${date}/250m/{z}/{y}/{x}.jpg`,
      wrapX: false
    }),
    visible: false // Default to not visible since this isn't the default base layer
  })
