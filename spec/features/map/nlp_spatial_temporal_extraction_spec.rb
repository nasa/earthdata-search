...
describe 'Spatial and temporal extraction' do
  ...
  context 'extracted temporal information' do
    ...
    it 'is set in the query' do
      expect(page.current_url).to include('qt=2018-12-01T00%3A00%3A00.000Z%2C2019-03-31T23%3A59%3A59.000Z&ok=last%20winter')
    end
  end
  ...
  context 'extracted spatial and temporal information' do
    ...
    it 'is set in the query and adds q= and ok= query params to the url' do
      expect(page.current_url).to have_content('qt=2018-12-01T00%3A00%3A00.000Z%2C2019-03-31T23%3A59%3A59.000Z&q=snow%20cover&ok=snow%20cover%20in%20Boston%20last%20winter&sb=-71.191155%2C42.22788%2C-70.748802%2C42.40082')
    end
