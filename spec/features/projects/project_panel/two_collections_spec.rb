...
describe 'Project with two collections' do
  context 'when viewing the project page', order: :defined do
    before :all do
      Capybara.reset_sessions!
      load_page :projects_page,  project: ['C1200187767-EDF_OPS', 'C1000000969-DEV08'], env: :sit, authenticate: 'edsc'
    end
    ...
    context 'when viewing the first panel' do
      ...
      it 'shows correct pagination' do
        within(activePanelOne) do
          ...
          expect(pagination).to have_content('Collection 1 of 2')
        end
      end
      ...
      context 'when clicking the next button' do
        ...
        context 'while the second panel is open' do
          ...
          it 'shows correct pagination' do
            within(activePanelTwo) do
              pagination = find('.master-overlay-panel-footer-page')
              expect(pagination).to have_content('Collection 2 of 2')
            end
          end
