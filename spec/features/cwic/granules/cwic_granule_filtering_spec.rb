...
describe "CWIC Granule list", reset: false do
  ...
  context "for all collections with granules" do
    ...
    context "clicking on the 'Filter granules' button" do
      ...
      it "does not display the search by granule id fields" do
        expect(page).to have_no_content("Granule ID")
      end

      it "displays the collection-specific temporal fields" do
        expect(page).to have_css("#temporal_start_button")
        expect(page).to have_css("#temporal_end_button")
      end

      it "does not display the search by day/night flag field" do
        expect(page).to have_no_content("Day / Night Flag")
      end

      it "does not display the search by data access fields" do
      ...
      it "does not display the search by cloud cover percentage fields" do
        expect(page).to have_no_content("Cloud Cover")
      end

      context "and selecting temporal range", pending_updates: true do
        before :all do
          ...
          page.find('#granule-filters-submit').click
          wait_for_xhr
        end
        ...
        it "has query param 'pg[0][qt]' in the url" do
          project_id = URI.parse(page.current_url).query[/^projectId=(\d+)$/, 1].to_i
