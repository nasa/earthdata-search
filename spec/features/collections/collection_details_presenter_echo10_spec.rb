...
describe CollectionDetailsPresenterEcho10 do
  before do
  ...
  it "converts difs" do
    ...
    presenter = CollectionDetailsPresenterEcho10.new(@collection)
  ...
  it "converts opensearch descriptive document (OSDD)" do
    ...
    @collection.version_id = 5
    @collection.archive_center = "LAADS"
    CollectionDetailsPresenterEcho10.new(@collection, 'C123-LAADS')
  ...
  it "converts contacts" do
    ...
    presenter = CollectionDetailsPresenterEcho10.new(@collection)
    expect(@collection.contacts).to eq([{:name=>"PLEASE CONTACT ORNL DAAC User Services", :phones=>["(865) 241-3952 (Direct Line)", "(865) 574-4665 (Fax)"], :email=>"ornldaac@ornl.gov"}])
  end
