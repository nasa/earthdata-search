...
describe "Limited Collections", reset: false do
  ...
  pending 'when the number of granules exceeds the collection limit (ASTER for example) in a project' do
    ...
    it 'displays tooltip on mouseover' do
      ...
      expect(page).to have_content('Due to significant processing times, orders for this collection are limited to 100 granules. Please narrow your search before downloading. Contact the data provider with questions. You can find contact information by clicking on the information icon.')
    end
  end
