...
describe Echo::Client do
  ...
  context 'collection details' do
    let(:collection_url) { "/search/concepts/C14758250-LPDAAC_ECS.echo10" }
  ...
  context 'collection facets' do
    ...
    it 'returns collection facets with a filter' do
      expect(connection).to receive(:get).with(collection_facets_url, {:campaign=>["AQUA"], :include_facets=>'v2', :page_size=>1, :options=>{:campaign=>{:and=>true}}, :include_has_granules=>true, :include_granule_counts=>true}).and_return(:response)
